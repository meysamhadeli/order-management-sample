...
public static class Extensions
{
    ...
    where TContext : DbContext, IDbContext
    {
        ...
        builder.Services.AddDbContext<TContext>(
            (sp, options) =>
            {
                string connectionString = sp.GetRequiredService<PostgresOptions>().ConnectionString;
    ...
    private static async Task SeedAsync(IServiceProvider serviceProvider)
    {
        ...
        var seedersManager = scope.ServiceProvider.GetRequiredService<IDataSeeder>();

        await seedersManager.SeedAllAsync();
    }
}
